          ...
          <table class="restock-orders-table">
            ...
            <tbody>
              <tr v-for="order in restockOrders" :key="order.id">
                  ...
                  <details class="items-details">
                    ...
                    <div class="items-dropdown">
                      <div v-for="item in order.items" :key="item.sku" class="item-entry">
                        ...
                        <span class="item-meta">{{ t('orders.quantity') }}: {{ item.quantity }} @ {{ currencySymbol }}{{ item.unit_cost.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
                      </div>
                    </div>
                  </details>
                </td>
                ...
                <td class="col-value"><strong>{{ currencySymbol }}{{ order.total_value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</strong></td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  setup() {
    ...
    // Restock orders are not affected by the global filters, so they are
    // loaded once on mount and never re-fetched on filter change.
    const loadRestockOrders = async () => {
      try {
        restockOrders.value = await api.getRestockOrders()
      } catch (err) {
        // Don't let a restock-orders failure break the main orders view
